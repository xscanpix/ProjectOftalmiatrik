...
The system allows for automatic server detection so that opticians don't have to manually connect by entering IP addresses. This allows for a quick and easy way to connect to a server. In order for the server detection to be considered working properly, it had to detect a server on a stable network at least 90\% of the time. With a success rate of 90\%, it was very uncommon to fail the detection twice in a row, while keeping the scanning time relatively low.
