...
For this project to be considered successful, the system must be able to display the visual acuity charts on a monitor properly. This means that the optotypes should be, depending on the distance the patient is from the screen, drawn with the correct size and position. The user have to be able to control which chart to display on the monitor from an Android device. The types of charts that should be included are the four Axanivis charts, the upper, the lower, the one line and the single optotype chart. The user also have to be able to decide which type of optotype the charts should use from the Sloan letters, Tumbling E, HVOT and LEA.
...
\section{Discussion}
\subsection{Advantages of our system}
\subsection{Disadvantages of our system}
