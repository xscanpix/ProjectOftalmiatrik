...
\subsection{Server application}
The server application is also written in Java and uses the built-in \textit{Swing} library for drawing the GUI and the images. The server will also handle the networking on its side, connecting wirelessly to the Android tablet. As the SVG vector graphic files are scalable by definition some of the code used on the Android tablet can be copied and used on the server. This includes at least the code generating the vector graphic files. Everything graphical in the server is generated at runtime as the sizes and distances between optotypes depends on which screen is connected and the distance from the screen to the patient. Instead of using the Android-only vector graphics library \textit{AndroidSVG} we are using a well supported library \textit{Apache Batik}. \cite{Batik}
