...
\section{Issues}
There are several technical problems that we need to solve. We need to find a suitable computer system that will act as the server, its job being storing and presenting the desired images to the screen. Our choice was to use a Raspberry Pi, since it is a cheap and compact alternative to a full-size computer that is more than powerful enough for our purposes. Although since the Raspberry Pi's CPU is running on an ARM-architecture there are a few compatibility problems. For example, the newly redesigned Java graphics library JavaFX, which is eventually going to replace the older Swing library, is only partially supported on ARM. Not being able to use the latest features can create problems with future Java updates when some things get deprecated. If Java Swing would become deprecated in the future it should not be too difficult to transfer the code to a compatible Java FX version.
